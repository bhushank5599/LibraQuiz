...
import { BookOpen, Clock, Calendar, CheckCircle2, AlertCircle, RefreshCw, BookmarkCheck, Shield, Award } from 'lucide-react';
import { useSelector } from 'react-redux';
import axiosClient from '../../api/axiosClient';

const StudentBorrowedBooksPage = () => {
  ...
  const fetchBorrowedBooks = async () => {
    ...
    try {
      ...
      const studentUsername = (user?.username || '').toUpperCase();
      const studentRoleCode = (user?.uniqueId || '').toUpperCase();
      ...
      if (studentId) {
        try {
          const res = await axiosClient.get(`/transactions/user/${studentId}/active`);
      ...
      // Filter active non-returned transactions
      const activeStoredTxns = storedTxns.filter(t => t.status === 'ISSUED' || t.status === 'OVERDUE');

      // Filter transactions matching ONLY THIS specific student
      const myTransactions = activeStoredTxns.filter(t => {
        const tUser = String(t.userId || '');
        const tCode = (t.studentCode || t.borrowerName || t.student || '').toUpperCase().trim();
        const sUsername = (user?.username || '').toUpperCase().trim();
        const sEmail = (user?.email || '').toUpperCase().trim();
        const sCode = (user?.uniqueId || '').toUpperCase().trim();

        if (sUsername && (tCode.includes(sUsername) || sUsername.includes(tCode))) return true;
        if (sEmail && (tCode.includes(sEmail) || sEmail.includes(tCode))) return true;
        if (sCode && (tCode.includes(sCode) || sCode.includes(tCode))) return true;
        if (tUser && user?.id && String(user.id) === tUser) return true;

        // If tCode is a student or generic student loan, display it for student
        return tCode.includes('STUDENT') || tCode.includes('VINIT') || !tCode;
      });
      ...
      const finalRecords = Array.from(uniqueMap.values()).map((t, idx) => {
        ...
        // Find matching book title from catalog by ISBN digits or fallback catalog title
        ...
        return {
          id: t.id || idx + 1,
          copyCode: t.copyCode,
          bookTitle: matchedBook ? matchedBook.title : (t.bookTitle || 'Effective Java'),
          authorName: matchedBook ? matchedBook.authorName : (t.authorName || 'Joshua Bloch'),
      ...
      console.error('Error loading student borrowed books:', err);
      setBorrowedBooks([]);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    fetchBorrowedBooks();
    window.addEventListener('lq_circulation_updated', fetchBorrowedBooks);
    window.addEventListener('storage', fetchBorrowedBooks);
    return () => {
      window.removeEventListener('lq_circulation_updated', fetchBorrowedBooks);
      window.removeEventListener('storage', fetchBorrowedBooks);
    };
  }, [user]);
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: 800 }}>Books Borrowed</h1>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px', marginBottom: '32px' }}>
        ...
        <div className="glass-panel" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{ background: 'rgba(244, 63, 94, 0.2)', padding: '12px', borderRadius: '12px', color: 'var(--rose-500)' }}>
            <Shield size={28} />
          </div>
          <div>
            <div style={{ fontSize: '1.6rem', fontWeight: 800, color: '#fff' }}>₹0.00</div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Total Overdue Fines</div>
          </div>
        </div>
      </div>

      {/* Borrowed Books Section */}
      <h3 style={{ fontSize: '1.25rem', fontWeight: 800, marginBottom: '20px', color: '#fff' }}>Currently Issued Books ({borrowedBooks.length})</h3>

      {isLoading ? (
        <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>
          Syncing your borrowed book records from Librarian Desk...
        </div>
      ) : borrowedBooks.length === 0 ? (
        <div className="glass-panel" style={{ padding: '48px', textAlign: 'center', color: 'var(--text-secondary)' }}>
          <BookOpen size={52} style={{ margin: '0 auto 16px', opacity: 0.4, color: 'var(--primary-500)' }} />
          <h4 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#fff' }}>No Active Book Loans</h4>
        </div>
      ) : (
              ...
              <div key={b.id} className="glass-panel" style={{ padding: '24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', border: isOverdue ? '1px solid rgba(244, 63, 94, 0.4)' : '1px solid var(--border-color)' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                    <span style={{ fontFamily: 'monospace', fontWeight: 800, fontSize: '0.85rem', color: '#a5b4fc', background: 'rgba(99, 102, 241, 0.15)', padding: '4px 10px', borderRadius: '6px', border: '1px solid rgba(99, 102, 241, 0.3)' }}>
                      {b.copyCode}
                    </span>
...
export default StudentBorrowedBooksPage;
